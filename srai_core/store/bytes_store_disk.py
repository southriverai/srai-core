...
class BytesStoreDisk(BytesStoreBase):
    ...
    def __init__(self, path_dir: str, extention: str) -> None:
        self.path_dir = path_dir
        self.extention = extention
        if not extention.startswith("."):
            raise ValueError(f"extention must start with a dot: {extention}")
        if not os.path.exists(self.path_dir):
            os.makedirs(self.path_dir)

    def get_path_file(self, bytes_id: str) -> str:
        return os.path.join(self.path_dir, bytes_id + self.extention)
    ...
    def load_bytes_all(self) -> Dict[str, bytes]:
        ...
        for file in os.listdir(self.path_dir):
            if file.endswith(self.extention):
                ...
                raise Exception(f"File {file} is not a {self.extention} file")
